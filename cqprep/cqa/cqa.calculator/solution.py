...
for n in range(num_lines):
    ...
    inp = input()
    r = inp.split(" ")
    data.append([int(r[0]), r[1], int(r[2])])

output = []

# DO PROCESSING HERE
for point in data:
    reg = round_advanced(eval(f"{point[0]}{point[1]}{point[2]}"), 1)
    rev = round_advanced(eval(f"{point[2]}{point[1]}{point[0]}"), 1)
    output.append(f"{reg} {rev}")
